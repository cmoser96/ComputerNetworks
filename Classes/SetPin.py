...
class SetPin:
    def __init__(self,headerpin=16,BCM="GPIO_23",direction="RX"):
        ...
        if BOARD2BCM[headerpin] != self.BCM:
            raise ValueError(
                "Header pin {} does not correspond to BroadCom GPIO {}".format(self.headerpin, self.BCM))
...
direction is  {}""".format(self.pin,self.BCM,self.in_out)
    ...
    def read_pin(self):
        return GPIO.input(self.headerpin)  # set 3.3V level on GPIO output
