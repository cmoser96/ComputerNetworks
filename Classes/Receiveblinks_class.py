...
class physical():
    def __init__(self, address):
        ...
        self.QT = queue.Queue(maxsize=500)
        self.fromMorse = {(1,1):'.',
                            (1,3):'-',
                            (0,1):'*',
                            (0,2):'_',
    ...
    def getdiff(self, time_stamp):
        return time.time() - time_stamp

    def receiveblinks(self, RXpin)->tuple:
        print("receiveblinks")
    ...
    def checksum(self, data):
        if data != None:
            ...
            for c in message:
                s += ord(c)
            if s%26 == ord(data[len(address)])-65:
                return data
            else:
                return None
        else:
    ...
    def checkBefore(self, speed, samplesperdit = 10):
        with SetPin(16,"GPIO_23",direction="RX") as RXpin:
            last_state = RXpin.read_pin()
            for i in range(9):
                if last_state != last_state:
                    return False
                delay(1/(speed*samplesperdit))
